...
public:
    ...
    void save_file() {
        ...
        for (int i=1; i<=10; i++) {
            _chunks[i] = _chunks[i]+"\n";
            f.write(_chunks[i].c_str(), _chunks[i].size());
        }
    ...
    void received_chunk(std::unique_ptr<FileChunk>&& chunk) {
        ...
        if (!_registered) {
            ...
            std::string chunk_str(chunk->data);
            size_t chunk_hash = *((size_t*)(&(chunk->data[chunk_str.length()+1])));

            if (get_hash(chunk_str) == chunk_hash) {
                // received chunk 
                _chunks[chunk_id] = chunk_str;
                _rcvd_chunks[chunk_id] = true;
                _num_rcvd_chunks++;
            }
            else {
                // chunk is corrupted
                std::cout << "Client received corrupted chunk " << chunk_id << std::endl;
                request_chunk(chunk_id);
            }

            if (_num_rcvd_chunks == 10 and !_saved_file) {
                save_file();
                // save_RTT_times();
                _saved_file = true;
            }
    ...
    void run(volatile bool& running) {
        ...
        while (running) {
            ...
            if (_saved_file) break;
            if (_can_request and _registered) {
                request_chunk(_next_chunk_idx);
                _next_chunk_idx = ((_next_chunk_idx+1)%10)+1;
            }
