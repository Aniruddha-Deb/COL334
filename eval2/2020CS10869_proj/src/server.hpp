...
public:
    ...
    */
    ...
    void received_chunk_request(uint32_t client_id, uint32_t chunk_id) {
        std::cout << "Received request for chunk " << chunk_id << std::endl;
        ...
        std::shared_ptr<FileChunk> f(new FileChunk());
        f->id = chunk_id;
        f->size = chunk.first.length();
        int i = 0;
        for (; i<chunk.first.length(); i++) {
            f->data[i] = chunk.first[i];
        }
        f->data[i++] = '\0';
        
        // send hash as well
        memcpy((f->data)+i, &chunk.second, sizeof(size_t));

        _clients[0]->send_chunk(f);
    ...
    void accept_new_client() {
        ...
        conn->send_control_msg({ REG, conn->get_client_id(), 11 });
